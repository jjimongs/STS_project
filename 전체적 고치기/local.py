...
class Local(metaclass=SingletonMeta):
    def __init__(self):
        ...
        self.set_meal_evening_listener()
        self.set_meal_morning_listener()
        self.set_meal_noon_listener()"""
    ...
    def play_sound_through_speaker(self, sound):
        ...
        sts = TextToSpeech().get_instance()
        sts.text_to_speech(sound)
        print(f"스피커에서 소리 출력: ")
    ...
    def printTodayWeather(self):
        ...
        if None in self.todayweather.values():
            ...
            self.tts.text_to_speech(weather_msg)
    ...
    def set_meal_morning_listener(self):
        ref = db.reference(f'UserAccount/{self.uid}/todoList/meal_morning')
        ref.listen(self.meal_listener)

    def set_meal_noon_listener(self):
        ref = db.reference(f'UserAccount/{self.uid}/todoList/meal_noon')
        ref.listen(self.meal_listener)

    def set_meal_evening_listener(self):
        ref = db.reference(f'UserAccount/{self.uid}/todoList/meal_evening')
        ref.listen(self.meal_listener)

    """def meal_listener(self,event):
        print(123)
        if event.data == 10:
            print(12)
            from STS.result1 import SpeechToText, TextToSpeech
            sts = SpeechToText().get_instance()
            tts = TextToSpeech.get_instance()
            print("1",sts.current_question)
            sts.current_question = "meal"
            print("2",sts.current_question)
            tts.text_to_speech("지금 밥 드시고 계신가요?")
            print(32)"""
            ...
            if file_info_dict:
                ...
                if 'fileName' in file_info_dict and 'fileUrl' in file_info_dict:
                    ...
                    if file_name and not os.path.exists(f'audio/{file_name}'):
                        ...
                        if match:
                            date_time_str = f"{match.group(2)}월 {match.group(3)}일 {match.group(4)}시 {match.group(5)}분 녹음입니다."
                            print(date_time_str)
    ...
    def addMood(self, mood):
        ...
        moodref.push({'date': curdate, 'mood': mood})
    ...
    def set_meal_morning(self):
        try:
            meal_ref = db.reference(f'UserAccount/{self.uid}/todoList/meal_morning')
            meal_ref.set(1)
        except Exception as e:
            print(f"Failed to set meal_morning status: {e}")

    def set_meal_noon(self):
        try:
            meal_ref = db.reference(f'UserAccount/{self.uid}/todoList/meal_noon')
            meal_ref.set(1)
        except Exception as e:
            print(f"Failed to set meal_noon status: {e}")
    def set_meal_evening(self):
        try:
            meal_ref = db.reference(f'UserAccount/{self.uid}/todoList/meal_evening')
            meal_ref.set(1)
        except Exception as e:
            print(f"Failed to set meal_evening status: {e}")

    def not_meal_morning(self):
        try:
            meal_ref = db.reference(f'UserAccount/{self.uid}/todoList/meal_morning')
            meal_ref.set(0)
        except Exception as e:
            print(f"Failed to not set meal_morning status: {e}")

    def not_meal_noon(self):
        try:
            meal_ref = db.reference(f'UserAccount/{self.uid}/todoList/meal_noon')
            meal_ref.set(0)
        except Exception as e:
            print(f"Failed to not set meal_noon status: {e}")
    def not_meal_evening(self):
        try:
            meal_ref = db.reference(f'UserAccount/{self.uid}/todoList/meal_evening')
            meal_ref.set(0)
        except Exception as e:
            print(f"Failed to not set meal_evening status: {e}")
    def set_medi(self):
        try:
            medi_ref = db.reference(f'UserAccount/{self.uid}/todoList/medicine')
            medi_ref.set(1)
        except Exception as e:
    ...
    def set_quiz(self):
        try:
            quiz_ref = db.reference(f'UserAccount/{self.uid}/todoList/quiz_completed')
            quiz_ref.set(1)
    ...
    def getAddress(self):
        ...
        if userdata:
            ...
            self.address = None
    def select_random_quizzes(self,num_quizzes=1):
