...
class Transcriber:
    def __init__(self):
        ...
        self.alone = 60                            # 혼자 1분동안 말이 없으면 혼자 뭐하세요 기능을 위한 시간 설정
    ...
    def transcribe_streaming(self):   # 실시간 사용자 음성 텍스트로 변환하는 함수
        ...
        while self.running:
            if not self.stream_use or self.is_speaking:  # 스트림 사용 중이지 않거나 GPT가 말하고 있는 경우 재설정을 기다림
                print("재설정을 기다립니다.")
                time.sleep(0.1)
                continue
    ...
    '''def alone_speech(self):
        
    def alone_speaking(self):
        if not self.stream_use and not self.is_speaking: 
            if time.time() - self.last_transcript_time > self.alone:'''
