        ...
        while self.running:
            ...
            if time.time() - self.last_transcript_time > self.silence_threshold:
                if not self.is_speaking:
                    print("5분 이상 대화 없어 메모리 초기화합니다.")
                    self.history.clear()
                    self.last_transcript_time = time.time()
            time.sleep(0.25)

    def alone_speaking(self):
        if time.time() - self.alone_last_transcript_time > self.alone and not self.is_speaking:
            print("혼자 뭐하세요?")  # 이 메시지가 출력되지 않는다면, 조건문이 False로 평가되고 있음
